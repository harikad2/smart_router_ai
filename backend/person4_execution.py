...
def run_execution(decision_output: DecisionOutput, analyzer_output: dict) -> ExecutionOutput:
    ...
    if len(items) != len(segments):
        print(
            f"  [warn] decision items ({len(items)}) != analyzer segments ({len(segments)}); "
            "aligning by index."
        )
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        for i, item in enumerate(items):
            seg_type = segments[i]["type"] if i < len(segments) else "general"
            fut = executor.submit(execute_one, item, seg_type, complexity)
            future_to_idx[fut] = i
